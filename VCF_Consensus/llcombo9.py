                ...
                if line_as_list == ['']:
                    ...
                    if svtype == "BND": 
                        ...
                        if "SUPPORT" in infodictionary:
                            ...
                            sample_stats = line_as_list[9]
                            #take allelic depth of alternate allele
                            allelic_depth_list = sample_stats.split(":")[1]
                            allelic_depth_list = allelic_depth_list.split(",")
                            allelic_depth_list = allelic_depth_list.pop(0)
                            allelic_depth = max(allelic_depth_list)
                            allelic_depth_index = allelic_depth_list.index(allelic_depth)
                            allelic_depth = int(allelic_depth)
                            # allelic_depth = int(allelic_depth.split(",")[1])
                        
                        #append to dictionary
                        if type(svlen) == list:
                            svlen = int(svlen[allelic_depth_index])
                ...
                for other in overlapdict:
                    if entry[1] != other[1]:
                        if sv != "BND" and overlaps(entry[0][2], other[0][2]) == True:
                            ...
                            print(f"{entry[0]}\t{other[0]}")
                            #if bnd type is the same
