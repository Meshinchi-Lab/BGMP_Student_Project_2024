...
for patient in patient_list:
    with open(f"testconsensus_no_filter_deduped.vcf", "w") as fo:
        ...
        for combo in chr_sv_combos:
            ...
            #initialize two dictionaries
                combodict2[caller] = {k:v for k,v in dictionary[caller].items() if k[0] == chrom and k[1] == sv}
            ...
            for entry in overlapdict:
                ...
                for i in overlapdict[entry]:
                    if i[0][3] > ad:
                        highest = i
                if highest not in highestlist:
                    highestlist.append(highest)
                


            for highest in highestlist:
                print(f"{entry}\t{highest[0][2]}\t{highest[1]}")
